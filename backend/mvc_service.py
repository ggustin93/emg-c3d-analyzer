...
class MVCEstimation:
    ...
    estimation_method: Literal['database', 'user_provided', 'clinical_estimation', 'signal_analysis']
...
class MVCService:
    ...
    async def _estimate_from_signal(
        ...
        """Clinical estimation of MVC from signal characteristics"""
        
        # Rectify signal for analysis
        rectified_signal = np.abs(signal_data)
        
        # Clinical method: 95th percentile represents strong voluntary contraction
        mvc_estimate = np.percentile(rectified_signal, 95)
        threshold_value = mvc_estimate * (threshold_percentage / 100.0)
        
        # Calculate confidence based on signal characteristics
        confidence_score = self._calculate_confidence(rectified_signal, mvc_estimate)
        
        # Metadata for clinical validation
        metadata = {
            'signal_length_seconds': len(signal_data) / sampling_rate,
            'signal_std': float(np.std(rectified_signal)),
            'signal_mean': float(np.mean(rectified_signal)),
            'percentile_95': float(mvc_estimate),
            'percentile_90': float(np.percentile(rectified_signal, 90)),
            'percentile_99': float(np.percentile(rectified_signal, 99)),
    ...
    async def bulk_estimate_mvc(
        ...
        for channel, signal_data in signal_data_dict.items():
            try:
                ...
                logger.error(f"MVC estimation failed for {channel}: {e}")
                # Provide fallback
