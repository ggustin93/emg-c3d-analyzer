...
class TestScoringConfigurationAPI:
    ...
    def test_get_scoring_configurations(self, mock_get_client, client, mock_supabase_response):
        ...
        mock_table.select.return_value.order.return_value.execute.return_value = Mock(
    ...
    @patch("api.routes.scoring_config.get_supabase_client")
    def test_get_active_scoring_configuration(
        self, mock_get_client, client, mock_supabase_response
    ):
    ...
    def test_create_scoring_configuration(self, mock_get_client, client, valid_config_request):
        ...
        assert not data["active"]  # New configs start inactive
    ...
    def test_activate_scoring_configuration(self, mock_get_client, client):
        ...
        mock_table.update.return_value.neq.return_value.execute.return_value = (
            Mock()
        )  # Deactivate all
        mock_table.update.return_value.eq.return_value.execute.return_value = Mock(
            data=[{"id": "test-config-123"}]
        )  # Activate specific
        mock_get_client.return_value = mock_client
    ...
    def test_get_custom_scoring_configuration_therapist_only(self, mock_get_client, client):
        ...
        mock_table.select.return_value.eq.return_value.is_.return_value.order.return_value.limit.return_value.execute.return_value = Mock(
            data=[therapist_config]
        )
        mock_get_client.return_value = mock_client
    ...
    def test_get_custom_scoring_configuration_therapist_patient(self, mock_get_client, client):
        ...
        mock_table.select.return_value.eq.return_value.eq.return_value.order.return_value.limit.return_value.execute.return_value = Mock(
            data=[patient_config]
        )
        mock_get_client.return_value = mock_client
        ...
        # Make request
        response = client.get(
            "/scoring/configurations/custom?therapist_id=therapist-123&patient_id=patient-456"
        )
    ...
    @patch("api.routes.scoring_config.get_supabase_client")
    def test_create_custom_scoring_configuration(
        self, mock_get_client, client, valid_config_request
    ):
        ...
        custom_request.update({"therapist_id": "therapist-123", "patient_id": "patient-456"})
    ...
    def test_create_custom_scoring_configuration_missing_therapist(
        self, client, valid_config_request
    ):
    ...
    @patch("api.routes.scoring_config.get_supabase_client")
    def test_update_existing_custom_configuration(
        self, mock_get_client, client, valid_config_request
    ):
        ...
        mock_table.select.return_value.eq.return_value.is_.return_value.execute.return_value = Mock(
            data=[{"id": "existing-config-123"}]
        )
