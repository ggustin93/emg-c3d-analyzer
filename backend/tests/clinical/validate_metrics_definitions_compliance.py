...
class MetricsDefinitionsValidator:
    ...
    def validate_default_weights(self) -> bool:
        ...
        if weights.w_compliance != expected["w_compliance"]:
            self.errors.append(f"❌ w_compliance: got {weights.w_compliance}, expected {expected['w_compliance']}")

        if weights.w_symmetry != expected["w_symmetry"]:
            self.errors.append(f"❌ w_symmetry: got {weights.w_symmetry}, expected {expected['w_symmetry']}")

        if weights.w_effort != expected["w_effort"]:
            self.errors.append(f"❌ w_effort: got {weights.w_effort}, expected {expected['w_effort']}")
        ...
        if abs(weights.w_completion - expected["w_completion"]) > 0.001:
            self.errors.append(f"❌ w_completion: got {weights.w_completion}, expected ~{expected['w_completion']:.3f}")

        if abs(weights.w_intensity - expected["w_intensity"]) > 0.001:
            self.errors.append(f"❌ w_intensity: got {weights.w_intensity}, expected ~{expected['w_intensity']:.3f}")

        if abs(weights.w_duration - expected["w_duration"]) > 0.001:
            self.errors.append(f"❌ w_duration: got {weights.w_duration}, expected ~{expected['w_duration']:.3f}")
    ...
    def validate_rpe_mapping(self) -> bool:
        ...
        test_cases = [
            # Optimal range (100%)
            (4, 100.0), (5, 100.0), (6, 100.0),
            # Acceptable range (80%)
            (3, 80.0), (7, 80.0),
            # Suboptimal range (60%)
            (2, 60.0), (8, 60.0),
            # Poor/dangerous range (20%)
            (0, 20.0), (1, 20.0), (9, 20.0), (10, 20.0)
        ]
    ...
    def validate_bfr_safety_gate(self) -> bool:
        ...
        bfr_test_cases = [
            # Compliant range
            (45.0, 1.0), (50.0, 1.0), (55.0, 1.0),
            # Non-compliant (below)
            (44.9, 0.0), (40.0, 0.0), (30.0, 0.0),
            # Non-compliant (above)
            (55.1, 0.0), (60.0, 0.0), (70.0, 0.0),
        ...
        for pressure, expected_gate in bfr_test_cases:
            ...
            if actual_gate != expected_gate:
                bfr_errors.append(f"❌ BFR {pressure}% AOP: got gate {actual_gate}, expected {expected_gate}")
    ...
    def validate_clinical_example(self) -> bool:
        ...
        expected_overall_compliance = (expected_left_compliance + expected_right_compliance) / 2  # 87.3%

        # Check compliance calculations
        if abs(compliance["left_muscle_compliance"] - expected_left_compliance) > 0.1:
            self.errors.append(f"❌ Left compliance: got {compliance['left_muscle_compliance']:.1f}%, expected {expected_left_compliance:.1f}%")

        if abs(compliance["right_muscle_compliance"] - expected_right_compliance) > 0.1:
            self.errors.append(f"❌ Right compliance: got {compliance['right_muscle_compliance']:.1f}%, expected {expected_right_compliance:.1f}%")

        if abs(compliance["overall_compliance"] - expected_overall_compliance) > 0.1:
            self.errors.append(f"❌ Overall compliance: got {compliance['overall_compliance']:.1f}%, expected {expected_overall_compliance:.1f}%")

        # Check symmetry calculation
        symmetry = self.service._calculate_symmetry_score(
            compliance["left_muscle_compliance"],
            compliance["right_muscle_compliance"]
        )
        expected_symmetry = (1 - abs(expected_left_compliance - expected_right_compliance) /
                           (expected_left_compliance + expected_right_compliance)) * 100  # 98.8%

        if abs(symmetry - expected_symmetry) > 0.1:
            self.errors.append(f"❌ Symmetry: got {symmetry:.1f}%, expected {expected_symmetry:.1f}%")

        # Check effort score (RPE = 6 → 100%)
        effort_score, _ = self.service._calculate_effort_score(clinical_metrics.rpe_post_session, False)
        ...
        game_score = self.service._calculate_game_score(
            clinical_metrics.game_points_achieved,
            clinical_metrics.game_points_max
        )
        if game_score != 85.0:
        ...
        result = self.service.calculate_performance_scores("clinical-example-validation", clinical_metrics)
        actual_overall = result.get("overall_score", 0)

        if abs(actual_overall - expected_overall) > 0.1:
            self.errors.append(f"❌ Overall performance: got {actual_overall:.1f}%, expected {expected_overall:.1f}%")
    ...
    def validate_formula_consistency(self) -> bool:
        ...
        if abs(actual_overall - expected_overall) > 0.1:
            self.errors.append(f"❌ Formula inconsistency: calculated {actual_overall:.1f}%, manual {expected_overall:.1f}%")
            return False
        else:
    ...
    def validate_single_source_of_truth(self) -> bool:
        ...
        if backend_weights.w_compliance != expected_frontend_fallback["compliance"]:
            consistency_errors.append(f"❌ Compliance weight mismatch: backend {backend_weights.w_compliance}, frontend fallback {expected_frontend_fallback['compliance']}")

        if backend_weights.w_symmetry != expected_frontend_fallback["symmetry"]:
            consistency_errors.append(f"❌ Symmetry weight mismatch: backend {backend_weights.w_symmetry}, frontend fallback {expected_frontend_fallback['symmetry']}")

        if backend_weights.w_effort != expected_frontend_fallback["effort"]:
            consistency_errors.append(f"❌ Effort weight mismatch: backend {backend_weights.w_effort}, frontend fallback {expected_frontend_fallback['effort']}")

        if backend_weights.w_game != expected_frontend_fallback["gameScore"]:
            consistency_errors.append(f"❌ Game weight mismatch: backend {backend_weights.w_game}, frontend fallback {expected_frontend_fallback['gameScore']}")
