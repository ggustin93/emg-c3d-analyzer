...
async def get_signal_data_jit(
    ...
    try:
        ...
        logger.error(f"❌ JIT signal generation failed: {str(e)}", exc_info=True)
...
async def get_available_channels(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"❌ Failed to get channels: {str(e)}", exc_info=True)
...
async def _extract_single_channel_jit(
    ...
    try:
        ...
        logger.error(f"❌ JIT extraction failed for {channel_name}: {str(e)}")
        return None
