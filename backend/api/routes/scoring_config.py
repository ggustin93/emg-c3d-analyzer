...
class ScoringConfigurationRequest(BaseModel):
    ...
    description: str | None = Field(None, max_length=500, description="Description of the configuration")
...
async def get_custom_scoring_configuration(
    ...
    try:
        ...
        logger.error(f"Failed to fetch custom scoring configuration: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def create_custom_scoring_configuration(config: ScoringConfigurationRequest):
    ...
    try:
        if not config.therapist_id:
            raise HTTPException(status_code=400, detail="therapist_id is required for custom configurations")
        ...
        if existing_result.data:
            ...
            result = supabase.table("scoring_configuration").update(config_data).eq("id", existing_result.data[0]["id"]).execute()
            logger.info(f"Updated custom scoring configuration for therapist {config.therapist_id}, patient {config.patient_id}")
        else:
            # Create new custom configuration
            result = supabase.table("scoring_configuration").insert(config_data).execute()
            logger.info(f"Created custom scoring configuration for therapist {config.therapist_id}, patient {config.patient_id}")

        if not result.data:
            raise HTTPException(status_code=400, detail="Failed to save custom scoring configuration")
