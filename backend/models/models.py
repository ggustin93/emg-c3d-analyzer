...
class GameSessionParameters(BaseModel):
    session_mvc_value: float | None = Field(None, description="Patient's MVC for this session/muscle, input by therapist")
    session_mvc_threshold_percentage: float | None = Field(DEFAULT_MVC_THRESHOLD_PERCENTAGE, ge=0, le=100, description="Percentage of session_mvc_value to consider a contraction 'good'")
    session_expected_contractions: int | None = Field(None, ge=0, description="Target number of contractions for the session")
    session_expected_contractions_ch1: int | None = Field(None, ge=0, description="Target number of contractions for channel 1")
    session_expected_contractions_ch2: int | None = Field(None, ge=0, description="Target number of contractions for channel 2")
    ...
    contraction_duration_threshold: int | None = Field(2000, ge=0, description="Global threshold in milliseconds to classify contractions as short or long")

    # Per-muscle duration thresholds (frontend sends in seconds, will be converted to ms)
    session_duration_thresholds_per_muscle: dict[str, float | None] | None = Field(None, description="Per-muscle duration thresholds in seconds (converted to ms for analysis)")

    # Channel to muscle name mapping
    channel_muscle_mapping: dict[str, str] | None = Field(None, description="Mapping of channel names to muscle names")

    # New fields for channel-specific MVC values and thresholds
    session_mvc_values: dict[str, float | None] | None = Field(None, description="Channel-specific MVC values")
    session_mvc_threshold_percentages: dict[str, float | None] | None = Field(None, description="Channel-specific MVC threshold percentages")
...
class EMGChannelSignalData(BaseModel):
    ...
    activated_data: list[float] | None = None # If you have a separate "activated" signal processing step
