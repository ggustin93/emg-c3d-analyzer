"""
C3D Reader Service for metadata extraction
Extracts technical and clinical metadata from C3D files
"""
import logging
...
class C3DReader:
    ...
    def _read_header(self, file_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error reading C3D header: {str(e)}")
            raise
    ...
    def _read_parameters(self, file_data: bytes, parameter_start: int) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error parsing parameter groups: {str(e)}")
            ...
            logger.error(f"Error reading C3D parameters: {str(e)}")
            return {}
    
    def _find_parameter_group(self, file_data: bytes, start_offset: int, group_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.warning(f"Error finding parameter group {group_name}: {str(e)}")
            return None
    ...
    def _extract_string_parameter(self, section_data: bytes, param_name: str) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.warning(f"Error extracting string parameter {param_name}: {str(e)}")
            return None