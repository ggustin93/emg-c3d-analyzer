...
class AbstractRepository(ABC, Generic[CreateModelType, UpdateModelType, ResponseModelType]):
    ...
    def create(self, data: CreateModelType) -> ResponseModelType:
        ...
        try:
            ...
            self.logger.info(f"✅ Created {self.get_table_name()}: {getattr(created_entity, 'id', 'unknown')}")
    ...
    def get_by_id(self, entity_id: str | UUID) -> Optional[ResponseModelType]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get {self.get_table_name()} by id {entity_id}: {e!s}")
            return None
    ...
    def update(self, entity_id: str | UUID, data: UpdateModelType) -> Optional[ResponseModelType]:
    ...
    def count(self, filters: Optional[Dict[str, Any]] = None) -> int:
        ...
        try:
            ...
            self.logger.error(f"Failed to count {self.get_table_name()}: {e!s}")
            return 0
    ...
    def bulk_create(self, data_list: List[CreateModelType]) -> List[ResponseModelType]:
        ...
        try:
            ...
            created_data = self._handle_supabase_response(result, "bulk create", self.get_table_name())
            
            # Convert to Pydantic models
            response_model = self.get_response_model()
            created_entities = [response_model.model_validate(item) for item in created_data]
            
            self.logger.info(f"✅ Bulk created {len(created_entities)} {self.get_table_name()} records")
            return created_entities
            
        except ValidationError as e:
            raise RepositoryError(f"Validation error in bulk create {self.get_table_name()}: {e}") from e
        except Exception as e:
