...
class UnifiedThresholdService:
    ...
    def __init__(self, mvc_service: Optional[MVCService] = None):
    ...
    async def get_channel_thresholds(
        ...
        try:
            ...
            logger.error(f"Error calculating unified thresholds for {channel}: {str(e)}")
            raise
    ...
    async def _get_mvc_with_fallback(
        ...
        try:
            ...
            logger.warning(f"Database MVC retrieval failed for {channel}: {str(e)}")
        ...
        if signal_data is not None and sampling_rate is not None:
            try:
                ...
                logger.warning(f"Signal MVC estimation failed for {channel}: {str(e)}")
    ...
    def _get_per_muscle_mvc(self, channel: str, session_params: GameSessionParameters) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(f"Error retrieving per-muscle MVC for {channel}: {str(e)}")
            return None
    ...
    def _get_mvc_percentage(self, channel: str, session_params: GameSessionParameters) -> float:
        ...
        try:
            ...
            logger.warning(f"Error retrieving MVC percentage for {channel}: {str(e)}")
    ...
    def _get_duration_threshold(self, channel: str, session_params: GameSessionParameters) -> int:
        ...
        try:
            ...
            logger.warning(f"Error retrieving duration threshold for {channel}: {str(e)}")
    ...
    def _get_muscle_name(self, channel: str, session_params: GameSessionParameters) -> str:
        ...
        try:
            ...
            logger.warning(f"Error retrieving muscle name for {channel}: {str(e)}")
            return channel
...
_unified_threshold_service: Optional[UnifiedThresholdService] = None
