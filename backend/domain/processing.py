...
def preprocess_emg_signal(
    ...
    custom_smoothing_window_ms: Optional[float] = None
) -> Dict:
    """
    ...
    if enable_filtering and ProcessingParameters.HIGHPASS_FILTER_ENABLED:
        try:
            ...
            if highpass_normalized >= 1.0:
                logger.warning(f"High-pass cutoff too high for sampling rate, skipping high-pass filtering")
            else:
            ...
            logger.error(f"High-pass filtering failed: {str(e)}")
            processing_steps.append(f"High-pass filtering FAILED: {str(e)}")
    ...
    if enable_filtering and ProcessingParameters.LOWPASS_FILTER_ENABLED:
        try:
            ...
            if cutoff_normalized >= 1.0:
                logger.warning(f"Cutoff frequency too high for sampling rate, skipping filtering")
            else:
            ...
            processing_steps.append(f"Filtering FAILED: {str(e)}")
...
__all__ = ['preprocess_emg_signal', 'validate_signal_quality', 'ProcessingParameters', 'get_processing_metadata']