...
-- Purpose: Get the appropriate scoring configuration for a therapy session
-- Priority: Session config → Patient config → Global default → Any active config
-- Parameters: 
--   p_session_id UUID - Session ID (optional)
--   p_patient_id UUID - Patient ID (optional)
-- Returns: UUID - Scoring configuration ID
-- Comment: Returns NULL only if no scoring configurations exist in the database
