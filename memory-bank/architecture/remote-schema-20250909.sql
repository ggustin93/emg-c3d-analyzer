...
CREATE TABLE public.clinical_notes (
  ...
  file_path text,
  patient_id uuid,
  content text NOT NULL CHECK (length(TRIM(BOTH FROM content)) >= 1 AND length(TRIM(BOTH FROM content)) <= 10000),
  note_type text NOT NULL CHECK (note_type = ANY (ARRAY['file'::text, 'patient'::text])),
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT clinical_notes_pkey PRIMARY KEY (id),
  CONSTRAINT clinical_notes_author_id_fkey FOREIGN KEY (author_id) REFERENCES auth.users(id),
