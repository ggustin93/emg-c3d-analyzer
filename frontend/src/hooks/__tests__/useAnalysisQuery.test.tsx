...
describe('useAnalysisQuery Cache Behavior', () => {
  ...
  describe('Error Handling', () => {
    it('should handle download service errors gracefully', async () => {
      const wrapper = createWrapper();
      
      // Reset mocks and setup error scenario
      vi.clearAllMocks();
      mockedSupabaseStorageService.downloadFile.mockRejectedValue(new Error('Download failed'));

      const params: AnalysisParams = {
        filename: 'error-test-file.c3d', // Use unique filename to avoid cache
        sessionParams: {}
      };
      ...
      expect(result.current.error?.message).toContain('Analysis failed');
    ...
    it('should handle API errors correctly', async () => {
      ...
      const params: AnalysisParams = {
        filename: 'api-error-test.c3d', // Use unique filename to avoid cache
        sessionParams: {}
      };
    ...
    it('should retry network errors but not client errors', async () => {
      const wrapper = createWrapper();
      ...
      const params: AnalysisParams = {
        filename: 'retry-test-file.c3d', // Use unique filename to avoid cache
        sessionParams: {}
      };
      ...
      // Verify the fetch was called (at least once, maybe not exactly once due to React Query internals)
